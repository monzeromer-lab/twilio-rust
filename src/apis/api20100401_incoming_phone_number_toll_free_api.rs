...
/// 
pub async fn create_incoming_phone_number_toll_free(configuration: &configuration::Configuration, params: CreateIncomingPhoneNumberTollFreeParams) -> Result<models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberTollFree, Error<CreateIncomingPhoneNumberTollFreeError>> {

    let uri_str = format!("{}/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers/TollFree.json", configuration.base_path, AccountSid=crate::apis::urlencode(params.account_sid));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateIncomingPhoneNumberTollFreeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// 
pub async fn list_incoming_phone_number_toll_free(configuration: &configuration::Configuration, params: ListIncomingPhoneNumberTollFreeParams) -> Result<models::ListIncomingPhoneNumberTollFreeResponse, Error<ListIncomingPhoneNumberTollFreeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListIncomingPhoneNumberTollFreeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
